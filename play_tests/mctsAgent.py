...
class treeNetAgent:
	...
	def evaluate(self, parent, state, inputState):
		...
		scores = np.reshape(self.model.predict(inputState),(169))
	...
	def search(self, time_budget = 1):
		...
		while(time.time() - startTime < time_budget):
			node, state, inputState = self.select_node()
			value = self.evaluate(node, state, inputState)
			turn = state.turn()
			self.backup(node, turn, value)
			num_evals += 1
		print("Ran "+str(num_evals)+ " evaluations in " +\
	...
	def select_node(self):
		...
		inputState = deepcopy(self.input)
		...
		while(len(node.children)!=0):
			...
			state.play(move)
			inputState = play_cell(inputState,self.pad(move),color)

		return (node, state, inputState)
